...
namespace proxything
{
	namespace util
	{
		 ...
		 * 
		 * A work-bound callback is bound to an asio::io_service::work instance, and
		 * thus prevents the asio::io_service from terminating before its associated
		 * task is completed.
		 */
		template<typename R, typename... Args>
		inline std::function<R(Args...)> work_bound(asio::io_service& service, std::function<R(Args...)> fn)
		{
			auto work = std::make_shared<asio::io_service::work>(service);
